...
const STRINGS = {
  en: {
    ...
    "install.trouble.oldvolume.q": "The installer refuses: \"its admin password is not recorded\"",
    "install.trouble.oldvolume.a": "There is a data volume from an earlier install in that folder, its .env is gone, and the volume's password was never recorded. OpenSearch keeps the password from its first boot, so installing over it would generate a new one and every request would answer 401 — that is why the installer refuses before installing, not after. Two ways out: restore one of the .env.backup.* files the installer kept in the folder, or re-run with --uninstall --purge-data to erase the index. Your documents and the journal on disk are the source, and Reconcile rebuilds it.",
  ...
  pt: {
    ...
    "install.trouble.oldvolume.q": "O instalador recusa: \"its admin password is not recorded\"",
    "install.trouble.oldvolume.a": "Existe um volume de dados de uma instalação anterior naquela pasta, o .env sumiu e a senha daquele volume nunca foi registrada. O OpenSearch guarda a senha da primeira subida, então instalar por cima geraria outra e toda requisição responderia 401 — por isso a recusa vem antes de instalar, não depois. Duas saídas: restaurar um dos .env.backup.* que o instalador guardou na pasta, ou rodar --uninstall --purge-data para apagar o índice. Seus documentos e o journal em disco são a fonte, e o Reconcile reconstrói.",
