...
const STRINGS = {
  en: {
    ...
    "install.trouble.oldvolume.a": "A data volume from an earlier install is there and its password was never recorded. OpenSearch keeps the password from its first boot, so a new install would generate another one and every request would answer 401. To start clean, re-run the installer with --uninstall --purge-data — your documents stay on disk and Reconcile rebuilds the index.",
  ...
  pt: {
    ...
    "install.trouble.oldvolume.a": "Existe um volume de dados de uma instalação anterior e a senha dele nunca foi registrada. O OpenSearch guarda a senha da primeira subida, então uma instalação nova geraria outra e toda requisição responderia 401. Para começar limpo, rode o instalador com --uninstall --purge-data — seus documentos ficam em disco e o Reconcile reconstrói o índice.",
