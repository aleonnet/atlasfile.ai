...
const STRINGS = {
  en: {
    ...
    "install.step4.body": "The interface opens at localhost:5173 with an onboarding wizard — paste the API key the installer printed (Settings → Access), create your first project, optionally add an OpenAI/Anthropic/Moonshot key for the assistant (validated live) — or pick the local Ollama model that was just set up — then drop files anywhere on the screen.",
  ...
  pt: {
    ...
    "install.step4.body": "A interface abre em localhost:5173 com um wizard de onboarding — cole a API key que o instalador imprimiu (Configuração → Acesso), crie seu primeiro projeto, adicione (opcional) uma chave OpenAI/Anthropic/Moonshot para o assistente (validada ao vivo) — ou escolha o modelo Ollama local recém-configurado — e solte arquivos em qualquer lugar da tela.",
